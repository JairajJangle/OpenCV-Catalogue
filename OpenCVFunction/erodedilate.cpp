...
ErodeDilate::ErodeDilate(MorphologyType morphologyType): morphType(morphologyType)
{
...
cv::Mat ErodeDilate::getProcessedImage(cv::Mat inputImage)try
{
...
void ErodeDilate::applyClicked(){
    kSize = cv::Size(kSizeDLEL->getTexts().first.toInt(),
                     kSizeDLEL->getTexts().second.toInt());
}
void ErodeDilate::resetClicked(){
    kSize = cv::Size(2 * dilationSize + 1, 2 * dilationSize + 1);
}
void ErodeDilate::morphShapeChanged(QVariant value){
    kernelMorphShape = value.toInt();
    qDebug() << "Morph Shape: " << kernelMorphShape;
}
void ErodeDilate::iterationChanged(int value){
    iterationCount = value;
}
void ErodeDilate::borderChanged(QVariant value){
    qDebug() << "Border Type: " << value;
    borderType = value.toInt();
}

void ErodeDilate::initWidget()
{
    kSizeDLEL = new DualLineEditLayout("Kernel Size",
                                       qMakePair(kSize.width,kSize.height),
                                       70);
    anchorLineEditLayout = new LineEditLayout("Current Anchor",
                                              "Default = (-1, -1)",
                                              160, 150);
    applyResetBL = new ApplyResetButtonLayout();
